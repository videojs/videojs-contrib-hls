 ...
 * @param {HlsHandler} hls we need hls here so that we can get xhr from it
  ...
  constructor(srcUrl, hls, withCredentials) {
    ...
    this.on('mediaupdatetimeout', () => {
      ...
      this.request = this.hls_.xhr({
        ...
        if (error) {
          return this.playlistRequestError_(this.request, this.media().uri);
        }
        this.haveMetadata_(this.request, this.media().uri);
      });
    });
  }

  /**
   * reusable function to trigger a playlist error and revert to
   * a startingState if it exists
   *
   * @param {Xhr} xhr an xhr object
   * @param {String} url the url to the updated playlist
   * @param {String} startingState the state before this action was taken
   */
  ...
  */
  ...
  media(playlist) {
    ...
    this.request = this.hls_.xhr({
      ...
      if (error) {
        return this.playlistRequestError_(this.request, playlist.uri, startingState);
      }

      this.haveMetadata_(req, playlist.uri);
  ...
  start() {
    ...
    this.request = this.hls_.xhr({
      ...
      this.haveMetadata_(req, this.srcUrl);
