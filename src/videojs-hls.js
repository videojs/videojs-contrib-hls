...
(function(window, videojs, document, undefined) {
  ...
  var
    ...
    bufferCheckInterval = 500, keyXhr, keyFailed, resolveUrl;
  ...
  videojs.Hls = videojs.Flash.extend({
    init: function (player, options, ready) {
      var source = options.source, settings = player.options();
      player.hls = this;
  ...
  videojs.Hls.prototype.src = function (src) {
    var tech = this, player = this.player(), settings = player.options().hls || {}, mediaSource, oldMediaPlaylist, source;
    ...
    this.playlists.on('loadedmetadata', videojs.bind(this, function () {
      var selectedPlaylist, loaderHandler, oldBitrate, newBitrate, segmentDuration, segmentDlTime, setupEvents, threshold;
      ...
      if (this.bandwidth === undefined) {
        ...
        this.setBandwidth({ bandwidth: this.playlists.bandwidth * 5 });
      }
      selectedPlaylist = this.selectPlaylist();
      oldBitrate = oldMediaPlaylist.attributes && oldMediaPlaylist.attributes.BANDWIDTH || 0;
      newBitrate = selectedPlaylist.attributes && selectedPlaylist.attributes.BANDWIDTH || 0;
      segmentDuration = oldMediaPlaylist.segments && oldMediaPlaylist.segments[this.mediaIndex].duration || oldMediaPlaylist.targetDuration;
 ...
 */
  videojs.Hls.getMediaIndexForLive_ = function (selectedPlaylist) {
    ...
    var tailIterator = selectedPlaylist.segments.length, tailDuration = 0, targetTail = (selectedPlaylist.targetDuration || 10) * 3;
  ...
  videojs.Hls.prototype.handleSourceOpen = function () {
    ...
    var player = this.player(), sourceBuffer = this.mediaSource.addSourceBuffer('video/flv; codecs="vp6,aac"');
  ...
  videojs.Hls.prototype.setupMetadataCueTranslation_ = function () {
    var tech = this, metadataStream = tech.segmentParser_.metadataStream, textTrack;
 ...
 */
  ...
  videojs.Hls.prototype.seekable = function () {
    ...
    return videojs.createTimeRange(startOffset, startOffset + (absoluteSeekable.end(0) - absoluteSeekable.start(0)));
  };
  /**
 * Update the player duration
 */
  videojs.Hls.prototype.updateDuration = function (playlist) {
    var player = this.player(), oldDuration = player.duration(), newDuration = videojs.Hls.Playlist.duration(playlist);
 ...
 */
  videojs.Hls.prototype.selectPlaylist = function () {
    var player = this.player(), effectiveBitrate, sortedPlaylists = this.playlists.master.playlists.slice(), bandwidthPlaylists = [], i = sortedPlaylists.length, variant, oldvariant, bandwidthBestVariant, resolutionPlusOne, resolutionBestVariant;
    ...
    while (i--) {
      ...
      if (variant.attributes.RESOLUTION.width === player.width() && variant.attributes.RESOLUTION.height === player.height()) {
        ...
        break;
      } else if (variant.attributes.RESOLUTION.width < player.width() && variant.attributes.RESOLUTION.height < player.height()) {
        // if we don't have an exact match, see if we have a good higher quality variant to use
        if (oldvariant && oldvariant.attributes && oldvariant.attributes.RESOLUTION && oldvariant.attributes.RESOLUTION.width && oldvariant.attributes.RESOLUTION.height) {
 ...
 */
  videojs.Hls.prototype.checkBuffer_ = function () {
    ...
    this.checkBufferTimeout_ = window.setTimeout(videojs.bind(this, this.checkBuffer_), bufferCheckInterval);
  };
  /**
 ...
 */
  videojs.Hls.prototype.fillBuffer = function (offset) {
    var player = this.player(), buffered = player.buffered(), bufferedTime = 0, segment, segmentUri;
    ...
    // played
    if (!this.playlists.media().endList && !this.player().hasClass('vjs-has-started')) {
      return;
    }
 ...
 */
  ...
  videojs.Hls.prototype.loadSegment = function (segmentUri, offset) {
    var tech = this, player = this.player(), settings = player.options().hls || {};
  ...
  videojs.Hls.prototype.drainBuffer = function (event) {
    var i = 0, segmentInfo, mediaIndex, playlist, offset, tags, bytes, segment, decrypter, segIv, ptsTime, segmentOffset = 0, segmentBuffer = this.segmentBuffer_;
    ...
    if (segment.key && !bytes) {
      ...
      if (keyFailed(segment.key)) {
        ...
        segIv = segment.key.iv || new Uint32Array([
          0,
          0,
          0,
        ...
        decrypter = new videojs.Hls.Decrypter(segmentInfo.encryptedBytes, segment.key.bytes, segIv, function (err, bytes) {
    ...
    if (tags.length > 0) {    
        //console.log("tags.length= " +tags.length + "tags[length].pts = " + tags[length].pts);
 ...
 */
  videojs.Hls.supportsNativeHls = function () {
    var video = document.createElement('video'), xMpegUrl, vndMpeg;
  ...
  videojs.Hls.isSupported = function () {
    ...
    return !videojs.Hls.supportsNativeHls && // Flash must be supported for the fallback to work
    videojs.Flash.isSupported() && // Media sources must be available to stream bytes to Flash
    videojs.MediaSource && // Typed arrays are used to repackage the segments
    window.Uint8Array;
  };
 ...
 */
  videojs.Hls.getPlaylistDuration = function (playlist, startIndex, endIndex) {
    videojs.log.warn('videojs.Hls.getPlaylistDuration is deprecated. ' + 'Use videojs.Hls.Playlist.duration instead');
 ...
 */
  videojs.Hls.getPlaylistTotalDuration = function (playlist) {
    videojs.log.warn('videojs.Hls.getPlaylistTotalDuration is deprecated. ' + 'Use videojs.Hls.Playlist.duration instead');
 ...
 */
  videojs.Hls.getMediaIndexByTime = function (playlist, time) {
    var index, counter, timeRanges, currentSegmentRange, segment, segmentDuration;    
    timeRanges = [];
    for (index = 0; index < playlist.segments.length; index++) {   
      ...
      currentSegmentRange.end = currentSegmentRange.start + segmentDuration;
 ...
 */
  resolveUrl = videojs.Hls.resolveUrl = function (basePath, path) {
    ...
    var oldBase = document.querySelector('base'), docHead = document.querySelector('head'), a = document.createElement('a'), base = oldBase, oldHref, result;
    // prep the document
    if (oldBase) {
